...
const store = new Vuex.Store({
  ...
  state: {
    isLoged: false, // se setea desde setSession
    userLoged: null,
    dolarBlue: null,
    lastFetchTime: null,
    partner: null,
  ...
  mutations: {
    ...
    SET_DOLAR_BLUE(state, value) {
      state.dolarBlue = value;
    },

    SET_SUNSET(state, sunset) {
      state.sunset = sunset;
    },
    
    /*
    setDolarBlue(state, dolarBlue) {
      state.dolarBlue = dolarBlue;
      window.localStorage.setItem('dolarBlue', JSON.stringify(dolarBlue));
    },
    setLastFetchTime(state, time) {
      state.lastFetchTime = time;
      window.localStorage.setItem('lastFetchTime', JSON.stringify(time));
    },*/
  ...
  actions: {
    setDolarBlue({ commit }, value) {
      commit('SET_DOLAR_BLUE', value);
    },

    setSunset({ commit }, sunset) {
      commit('SET_SUNSET', sunset);
    },
    /*
    async fetchDolarBlue({ commit }) {
      try {
        const response = await axios.get('https://www.dolarsi.com/api/api.php?type=valoresprincipales');
        const dolarBlue = response.data.find(item => item.casa.nombre === 'Dolar blue');
        commit('setDolarBlue', dolarBlue);
    ...
    */
    ...
    login({ commit }, { username, password }) {
      return axios.post('http://localhost:8082/api/session/login', {
        username: username,
        password: password
      })
      .then(response => {
