...
export function testInternetConnection(){
    return new Promise((resolve) => {
      /*process.env.VUE_APP_API_URL_ALARMS*/
      axios.post(process.env.VUE_APP+'/connection', {}, { timeout: 5000 })
        .then(function() {
          resolve(true);
        }).catch(function(){
          resolve(false);
...
export function isMozilla() {
  const userAgent = navigator.userAgent.toLowerCase();
  return userAgent.indexOf('mozilla') !== -1 && userAgent.indexOf('firefox') !== -1;
}
...
/* solicitar que el elemento document.body entre en modo de pantalla completa.
Esta función comprueba qué método de solicitud de pantalla completa es compatible con el navegador y llama a la función correspondiente para solicitar el modo de pantalla completa.
Puedes llamar a esta función para activar el modo de pantalla completa de la siguiente manera:
*/
...
export function imageEmp(emp = null) {
  ...
  if (emp) {
    ...
    /*process.env.VUE_APP_URL_YO */
    return process.env.VUE_APP + "/" + emp.Emp_Imagen;
  }
  return null;
}
