...
export default {
  ...
  mounted() {
    ...
    if (id) {
        this.id = id
        this.$http.get(`http://localhost:8082/api/pilots/${id}`)
          .then(response => {
  ...
  methods: {
    savePilot() {
      this.loading = true;
      const url = this.id ? `http://localhost:8082/api/pilots/${this.id}` : 'http://localhost:8082/api/pilots';
    ...
    deletePilot(){
      if (this.id) {
        this.loading = true;
        this.$http.delete(`http://localhost:8082/api/pilots/${this.id}`)
          .then(response => {
